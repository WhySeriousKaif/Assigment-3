...
export default function ChatPage() {
  ...
  return (
    <div className="h-screen bg-neutral-950 text-neutral-50 flex flex-col overflow-hidden selection:bg-indigo-500/30 pt-16">
      <Navbar />
      
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
        
        {/* Left Side: PDF Viewer & Upload */}
        <div className={`w-full ${isPdfExpanded ? 'md:w-3/4' : 'md:w-1/2'} border-r border-neutral-800 flex flex-col bg-neutral-900/50 overflow-hidden transition-all duration-300`}>
          ...
          <div className="h-12 border-b border-neutral-800 flex items-center justify-between px-4 bg-neutral-900 shrink-0">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setIsPdfExpanded(!isPdfExpanded)} className="p-1 hover:bg-neutral-800 rounded ml-2">
                {isPdfExpanded ? <Minimize2 className="h-4 w-4" /> : <Maximize2 className="h-4 w-4" />}
              </button>
              
              <label className="ml-4 bg-indigo-600 hover:bg-indigo-700 text-[10px] uppercase tracking-wider font-bold px-3 py-1 rounded cursor-pointer transition-colors flex items-center">
                <input 
                  type="file" 
                  accept=".pdf"
                  className="hidden" 
        ...
        {/* Right Side: Chat */}
        <div className={`w-full ${isPdfExpanded ? 'md:w-1/4' : 'md:w-1/2'} flex flex-col bg-neutral-950 overflow-hidden transition-all duration-300`}>
